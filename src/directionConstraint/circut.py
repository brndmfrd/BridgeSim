...
Q = numpy.zeros((4,4))
'''
The direction on the CS each car/process would like to take
w := west-bound
e := east-bound
n := north-bound
s := south-bound
'''
my_direction = ['e', 'e', 'e', 'e']
...
class Car(pygame.sprite.Sprite):
    ...
    def update(self, v):
        ...
        if self.in_cs:
            ...
            if self.at_exit:
                self.in_cs = False
                ''' signify we are out of the CS, no other awk at this time '''
        ...
        if self.req and (not self.in_cs):
            ...
            if (Q[self.car_id, 0] > float(-1)) and (Q[self.car_id, 1] > float(-1)) and (Q[self.car_id, 2] > float(-1)) and (Q[self.car_id, 3] > float(-1)):
                minTimestamp = numpy.amin(Q[self.car_id])
                myTimestamp = Q[self.car_id, self.car_id]
                if (minTimestamp == myTimestamp):
                    self.in_cs = True
        ...
        if self.in_cs and (not self.at_entrance) and (not self.at_exit):
            ''' allow other in the same direction on the bridge/cs '''
            if (Q[self.car_id, 0] > -1) and (my_direction[0] == my_direction[self.car_id]):            
                Q[0,0] = Q[self.car_id, self.car_id]
            
            if (Q[self.car_id, 1] > -1) and (my_direction[1] == my_direction[self.car_id]):
                Q[1, 1] = Q[self.car_id, self.car_id]
            
            if (Q[self.car_id, 2] > -1) and (my_direction[2] == my_direction[self.car_id]):
                Q[2, 2] = Q[self.car_id, self.car_id]
            
            if (Q[self.car_id, 3] > -1) and (my_direction[3] == my_direction[self.car_id]):
                Q[3, 3] = Q[self.car_id, self.car_id]
